...
<html>
	...
	<div class="row" >
		<div class="col-lg-6 bigDiv">
            ...
            @csrf
            @method('PUT')
				<input type="text" name="name" class="form-input" placeholder ="Enter Name of the Company" value="{{ $company->name }}">
				@error('name')
                    <p class="help is-danger">{{ $errors->first('name')}}</p>
                @enderror
				<br>
				<input type="text" name="address" placeholder="Enter Address" class="form-input" value="{{ $company->address }}">
				@error('address')
                    <p class="help is-danger">{{ $errors->first('address')}}</p>
                @enderror
				<input type="text" name="telephone" placeholder="Enter Telephone number" class="form-input" value="{{ $company->telephone }}">
				@error('telephone')
                    <p class="help is-danger">{{ $errors->first('telephone')}}</p>
                @enderror
				<input type="date" name="open_date" placeholder="Enter Open Date" class="form-input" value="{{ $company->open_date }}">
				@error('open_date')
                    <p class="help is-danger">{{ $errors->first('open_date')}}</p>
				@enderror
				<input type="date" name="end_date" placeholder="Enter End Date" class="form-input" value="{{ $company->end_date }}">
				@error('end_date')
